...
/*
	Delta transport, @see model-api/src/model/deltas/deltas.ts
...
export interface ICreateElementDelta extends IElementDelta {
	...
	// TODO  add additionIndex for listy parent properties?
...
export interface IMutator {
	 ...
	 */
	mutatorType: string;	 // -> MutatorTypes
...
export interface IMoveMutator extends IMutator {
	fromIndex: number; // XXX: figure this one out
	toIndex: number;
}
...
export interface IProblem {
	name: string; // The name of the problem (consistency check), as it is described in the metamodel.
...
export interface IProblemLocation {
	elementId?: string;
	unitId?: string;
	element?: string; // element described as text
	document?: string; // unit / document described as text
	module?: string;
}

export type IUpdateAppJobStatus = "updating" | "resuming" | "provisioning" | "started" | "consistencyerrors" | "failed";
...
export interface IDeployJobStatus {
	...
	buildstatus: string;
	consistencyErrors?: IProblem[]; // parsed by SDK
	appUrl?: string;
	/**
